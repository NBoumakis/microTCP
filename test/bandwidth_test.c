...
static inline void print_statistics(ssize_t received, struct timespec start,
                                    struct timespec end) {
    double elapsed =
    ...
    while ((opt = getopt(argc, argv, "hsmf:p:a:")) != -1) {
        ...
        default:
            printf("Usage: bandwidth_test [-s] [-m] -p port -f file"
                   ...
                   "Otherwise as client.\n"
                   "   -m                  If set, the program uses the microTCP "
                   "implementation. Otherwise the normal TCP.\n"
                   "   -f <string>         If -s is set the -f option specifies the "
                   "filename of the file that will be saved.\n"
                   "                       If not, is the source file at the client "
                   "side that will be sent to the server.\n"
                   "   -p <int>            The listening port of the server\n"
                   "   -a <string>         The IP address of the server. This "
                   "option is ignored if the tool runs in server mode.\n"
                   "   -h                  prints this help\n");
            exit(EXIT_FAILURE);
        }
    }
