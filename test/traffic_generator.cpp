...
int main(int argc, char **argv) {
    ...
    while ((opt = getopt(argc, argv, "hp:i:")) != -1) {
        ...
        case 'i':
            ...
            break;
        default:
            printf("Usage: bandwidth_test -p port -i packet inter-arrival ms"
                   "Options:\n"
                   "   -p <int>            the port to wait for a peer"
                   "   -i <int>            the mean inter-arrival time in "
                   "milliseconds of the poisson distribution"
                   "   -h                  prints this help\n");
            exit(EXIT_FAILURE);
        }
    }
    ...
    if (microtcp_bind(&sock, (struct sockaddr *)&sin, sizeof(struct sockaddr_in)) ==
        -1) {
